...
2. **Cache miss?** Verify KV credentials, check `X-Cache-Source` header
3. **Wrong dates?** Always use `date-utils.ts` functions
4. **ICS events not showing?** See [`ICS_CALENDAR_GUIDE.md`](../markdown_files/ICS_CALENDAR_GUIDE.md) troubleshooting
5. **Cron failing?** Check Vercel logs, verify `CRON_SECRET`
6. **AI timeout?** Increase `maxDuration` in route exportseColor()`
4. Commit + push (auto-deploys to Vercel)
5. Verify: `curl https://oski.app/api/unified-dashboard?refresh=true`

**Key patterns:**
- ❌ Week URLs (`/pages/week-X`) → ✅ Course home URLs
- ICS files must be in `public/` directory
- Dev mode bypasses cache for calendar data (auto-updates)
