...
|---|---|---|
| Cal Athletics | `https://calbears.com/calendar.ics` (official composite feed) | Keep **home Berkeley events only** (drop away + neutral-site games). Severity by venue: football @ Memorial Stadium = HIGH; Haas Pavilion = MEDIUM; other Berkeley venues = LOW (icon only, no badge). |
| Greek Theatre | Scrape `https://thegreekberkeley.com/` homepage + `/events/{slug}` pages (no official feed exists) | Concerts only — MEDIUM. UC non-concert events are never listed here (known gap). |
...
- UI rule: only HIGH and MEDIUM severity events trigger the parking badge and
